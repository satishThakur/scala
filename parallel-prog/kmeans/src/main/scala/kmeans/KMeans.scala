...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    ...
    means.map(p => p -> grouped.getOrElse(p, List())).toMap

  }

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    val grouped = points.groupBy(p => findClosest(p, means))
    means.map(p => p -> grouped.getOrElse(p, List().par)).toMap
  }
  ...
  def converged(eta: Double, oldMeans: Seq[Point], newMeans: Seq[Point]): Boolean = {
    oldMeans.zip(newMeans).forall(ps => ps._1.squareDistance(ps._2) <= eta)
  ...
  final def kMeans(points: Seq[Point], means: Seq[Point], eta: Double): Seq[Point] = {
    val cfy = classify(points, means)
    val newMeans = update(cfy, means)
