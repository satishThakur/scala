...
package object barneshut {
  ...
  case class Fork(
    ...
    val massY: Float = if(mass == 0) centerX
    ...
    def insert(b: Body): Fork = {
      ...
      if(b.x >= centerX && b.y <= centerY) Fork(nw, ne.insert(b), sw, se)
  ...
  extends Quad {
    val mass = bodies.foldLeft(0.0f)( (acc, b) => acc + b.mass) : Float
    val massX =  bodies.foldLeft(0.0f)( (acc, b) => acc + b.mass * b.x )/mass : Float
    val massY = bodies.foldLeft(0.0f)( (acc, b) => acc + b.mass * b.y )/mass : Float

    val total: Int = bodies.length
    def insert(b: Body): Quad = {
      if(size < minimumSize){
        Leaf(centerX, centerX, size, b +: bodies)
      }else{
        val qtr = size / 4
        val nw = Empty(centerX - qtr, centerY - qtr, size / 2)
        val ne = Empty(centerX + qtr, centerY - qtr, size / 2)
        val sw = Empty(centerX - qtr, centerY + qtr, size / 2)
        val se = Empty(centerX + qtr, centerY + qtr, size / 2)
