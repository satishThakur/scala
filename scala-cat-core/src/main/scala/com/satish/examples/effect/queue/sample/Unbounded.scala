package com.satish.examples.effect.queue.sample
...
object ProducerConsumer extends IOApp {
  ...
  def producer[F[_]: Sync: Console](id: Int, counterR: Ref[F, Int], stateR: Ref[F, State[F,Int]]): F[Unit] = {
    ...
    for {
      ...
      _ <- offer(i)
      _ <- if(i % 10000 == 0) Console[F].println(s"[${Thread.currentThread().getName}] Producer $id has reached $i items") else Sync[F].unit
  ...
  def consumer[F[_]: Async: Console](id: Int, stateR: Ref[F, State[F, Int]]): F[Unit] = {
    ...
    for {
      i <- take
      _ <- if(i % 10000 == 0) Console[F].println(s"[${Thread.currentThread().getName}] Consumer $id has reached $i items") else Async[F].unit
