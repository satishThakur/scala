...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    b.legalNeighbors.foldLeft[LazyList[(Block, List[Move])]](LazyList.empty)( (s,bh) => {
      val (b,m) = bh
      (b, m::history) #::s
    })
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if(initial.isEmpty) LazyList.empty
      ...
      val next: LazyList[(Block, List[Move])]  = for {
        (b, moved) <- initial
        n <- newNeighborsOnly(neighborsWithHistory(b, moved), explored)

      } yield n

      initial #::: from(next,explored ++ (next map(n => n._1)))
    }
  }
  ...
  lazy val solution: List[Move] = {
    ...
    if(paths.isEmpty) Nil
      ...
      paths.headOption.get._2.reverse
    }
  }
}
