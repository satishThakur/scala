...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      json => json match {
          case Json.Arr(ss: List[Json]) => {
            val x: List[Option[A]] = ss.map(s => decoder.decode(s))

            x.foldRight[Option[List[A]]](Some(Nil))((a, r) => (a, r) match {
              case (_, None) => None
              case (None, _) => None
              case (Some(av), Some(l)) => Some(av :: l)
            })
          }
          case _ => None
        }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction{
      json => json match {
        case Json.Obj(obj) => obj.get(name).flatMap(j => decoder.decode(j))
        case _ => None
      }
    }
}
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    ...
    .transform[Person](n => Person(n._1,n._2))
...
trait ContactsCodecs {
  ...
  implicit val contactDecoder: Decoder[Contacts] =
    Decoder.field[List[Person]]("people").transform(l => Contacts(l))
