...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    langs.map(l => (l, occurrencesOfLang(l,rdd))).sortWith( (p1, p2) => p1._2 > p2._2)

  /* Compute an inverted index of the set of articles, mapping each language
   * to the Wikipedia pages in which it occurs.
   */
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap(article => langs.filter(l => article.mentionsLanguage(l)).map(lang => (lang, article)) ).groupByKey()
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    val a: RDD[(String, Int)] = rdd.flatMap(article => langs.filter(l => article.mentionsLanguage(l)).map(lang => (lang, 1)) )
    a.reduceByKey(_ + _).sortBy(_._2, false).collect().toList
  }
