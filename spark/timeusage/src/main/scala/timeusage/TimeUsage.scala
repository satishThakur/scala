...
object TimeUsage extends TimeUsageInterface {
  ...
  def row(line: List[String]): Row = {
    val values: Seq[Any] = line.head :: line.tail.map(s => s.toDouble)
    Row.fromSeq(values)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val workActPrefixes = List("t05", "t1805")
    val otherActPrefixes = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

    def isPrimaryNeed(s: String): Boolean = primaryNeedPrefixes.exists(s.startsWith)
    def isWorkActPrefixes(s: String): Boolean = workActPrefixes.exists(s.startsWith)
    def isOtherActPrefixes(s: String): Boolean = otherActPrefixes.exists(s.startsWith)

    columnNames.foldLeft((List[Column](), List[Column](), List[Column]()))( (z,c) => c match {
      case cv if isPrimaryNeed(cv) => (column(cv) :: z._1, z._2, z._3)
      case cv if isWorkActPrefixes(cv) => (z._1, column(cv) :: z._2, z._3)
      case cv if isOtherActPrefixes(cv) => (z._1, z._2, column(cv) :: z._3)
      case _ => (z._1, z._2, z._3)
    })
  }
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = when(col("teage").between(15,23), "young") when(col("teage").between(23,55), "active") otherwise "elder" as "age"
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(row => (row.working, row.sex, row.age))
      .agg(round(typed.avg[TimeUsageRow](_.primaryNeeds),1).as("primaryNeeds").as[Double],
        round(typed.avg[TimeUsageRow](_.work),1).as("work").as[Double],
        round(typed.avg[TimeUsageRow](_.other).as("other"),1).as[Double])
    .map(r => TimeUsageRow(
      r._1._1,
      r._1._2,
      r._1._3,
