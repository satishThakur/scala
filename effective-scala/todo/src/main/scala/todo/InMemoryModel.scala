...
object InMemoryModel extends Model:
  ...
  def delete(id: Id): Boolean =
    var found = idStore.remove(id).isDefined
    found
  ...
  def tags: Tags = {
    val allTags : List[Tag] = idStore.values.map(_.tags).foldRight(List.empty)( (l1, l2) => l1 ++ l2)
    Tags(allTags.distinct)
  }

  def tasks(tag: Tag): Tasks =
    Tasks(idStore.filter((_, t) => t.tags.contains(tag)))
