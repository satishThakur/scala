...
object PersistentModel extends Model:
  ...
  def create(task: Task): Id =
    val id = loadId()
    val tasks = loadTasks()
    saveTasks(Tasks(tasks.toList ++ List((id, task))))
    saveId(id.next)
    id
  ...
  def delete(id: Id): Boolean =
    val tasks = loadTasks()
    val updatedTasks = tasks.toList.filter(t => t(0) != id)
    saveTasks(Tasks(updatedTasks))
    tasks.toMap.get(id).isDefined

  def tasks: Tasks = loadTasks()


  def tasks(tag: Tag): Tasks =
    Tasks(tasks.toList.filter((_, t) => t.tags.contains(tag)))

  def complete(id: Id): Option[Task] =
    update(id)(t => t.complete)

  def tags: Tags =
    val allTags : List[Tag] = tasks.toList.map(_(1).tags).foldRight(List.empty)( (l1, l2) => l1 ++ l2)
