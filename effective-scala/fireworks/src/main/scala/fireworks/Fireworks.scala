...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework:
   ...
   */
  def next: Firework =
    if countDown > 0 then copy(countDown = countDown - 1, position = Motion.movePoint(position, direction, Settings.propulsionSpeed))
...
case class Exploding(countDown: Int, particles: Particles) extends Firework:
   ...
   */
  def next: Firework =
    if countDown > 0 then copy(countDown = countDown - 1, Particles(particles.value.map(_.next)))
    else Done
